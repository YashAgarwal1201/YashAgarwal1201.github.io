...
function Home({ home, quoteVar }) {
  ...
  return (
    <>
      <Layout title={title} content={content} imagePath={imagePath}>
        {" "}
      </Layout>
    </>
  );
...
function Work({ work }) {
  ...
  const content = Object.values(work.content).map((value, key) =>
    contentF(value, key)
  );
  return (
    <>
...
function Feedback({ feedback }) {
  ...
  const imagePath = (
    <img src={feedback.imagePath} href="image for feedback section" />
  );
    ...
    <div className={`form-section`}>
      ...
      >
        <div className={formDivStyles}>
          <label className={formLabelStyles}>Your Email Address:</label>
          <br />
          <input className={formInputStyles} type="email" name="Email" />
        </div>
        <div className={formDivStyles}>
          <label className={formLabelStyles}>Your Name:</label>
          <br />
          <input className={formInputStyles} type="text" name="Name" />
        </div>
        <div className={formDivStyles}>
          <label className={formLabelStyles}>Your Message:</label>
          <br />
          <textarea className={formInputStyles} name="Msg"></textarea>
        </div>
        <div className={formDivStyles}>
          <button className={formBtnStyles} type="submit">
            send
          </button>
          <button className={formBtnStyles} type="reset">
            delete
          </button>
        </div>
        <p>
...
function RightClickMenu() {
  ...
  return (
    <div
      className="rightClickMenu rightClickMenu-hover w3-theme-l2 w3-card w3-round-large"
      id="rightClickMenu"
    >
      <span className={rightClickMenuStyles} onClick={() => location.reload()}>
        <i className="material-icons-outlined">refresh</i>Reload
      </span>
      <span className={rightClickMenuStyles} onClick={() => history.go(1)}>
        <i className="material-icons-outlined">arrow_forward</i>Forward
      </span>
      <span className={rightClickMenuStyles} onClick={() => history.go(-1)}>
        <i className="material-icons-outlined">arrow_back</i>Back
      </span>
      <span
        className={rightClickMenuStyles}
        onClick={() => {
      ...
      >
        <i className="material-icons-outlined">more</i>Default Menu
      </span>
    </div>
  );
}
  ...
  return (
    ...
    >
      <span
        ...
        className="w3-button w3-display-topright w3-round w3-theme-l1 w3-margin"
      >
        &times;
      </span>
...
const Component = () => {
  useEffect(() => {
    ...
    if (
      ...
      document
        .querySelectorAll("*")
        .forEach((item) => item.classList.remove("[class*=hover]"));
    }
  }, []);
  ...
  return (
    <>
      ...
      <div className={`w3-margin-bottom main-content`} id="main-content-id">
        {clickState === "home" ? (
          <Home home={home} quoteVar={quoteObj} />
        ) : clickState === "about" ? (
          <About about={about} />
        ) : clickState === "work" ? (
          <Work work={work} />
